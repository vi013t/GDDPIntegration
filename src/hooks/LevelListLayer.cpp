...
/*class $modify(LevelListLayer) {
	...
	virtual void onBack(CCObject * sender) {
		...
		if (inGDDP && !Mod::get()->getSavedValue<bool>("is-practice", false)) {
			...
			auto listID = data[type][id]["listID"].as_int(); //Only used for obtaining old data
			std::string saveID = "beginner";

			if (type == "monthly") { saveID = fmt::format("{}-{}", data[type][id]["month"].as_int(), data[type][id]["year"].as_int()); }
			else { saveID = data[type][id]["saveID"].as_string(); }
			...
			Mod::get()->setSavedValue<ListSaveFormat>(saveID, ListSaveFormat { .progress = packProgress, .completed = completed, .hasRank = hasRank });
		}
	}
